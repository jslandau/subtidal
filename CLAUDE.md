...
- **Process source enumeration** (`list_sources` on macOS): returns one `System Output` row first, then deterministic user-facing app rows. Raw Core Audio process bundle IDs are filtered/grouped so helper-heavy apps (Discord/Slack/Zoom/browsers) appear under the visible parent app while their raw helper IDs are retained in `AudioSource::App.capture_bundle_ids` for tap construction. The low-level Core Audio enumeration remains raw and **does not** filter by `kAudioProcessPropertyIsRunning` — that property reflects "audio I/O active at this instant" and would hide apps mid-pause.
