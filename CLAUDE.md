...
- **macOS overlay panel structure** (`overlay/macos/panel.rs`): the NSPanel's contentView is a layer-backed wrapper `NSView` carrying the rounded translucent `CALayer` (corner radius + background color). The caption `NSTextField` is a subview inset by `INSET=10`pt with flexible width+height autoresizing. `apply_background_color` operates on the wrapper view; `apply_geometry` takes `&NSTextField` directly (downcasting from contentView is no longer possible and previously contributed to a Docked-switch crash). Panel height uses `font_size * 1.3 * lines + 16`. Docked mode centers a `config.appearance.width`-wide panel at the top of `visibleFrame` with Center alignment; Floating uses Left alignment. `set_caption_text` resizes the panel to `min(actual_lines, max_lines)` with the top edge held fixed *before* `setStringValue` to avoid resize-during-paint flicker. `setIgnoresMouseEvents` in Floating mode is tied to lock state (true when locked).
