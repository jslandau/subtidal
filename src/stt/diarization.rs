...
//! inference pass. With the current low-latency preset (`chunk_len=6`,
//! `right_context=7`), algorithmic diarization latency is about 1.04 seconds;
//! larger chunk/context settings trade more delay for potentially better
//! speaker-boundary stability.
//!
//! # Platform dispatch
//!
...
/// reducing latency.
const RT_CHUNK_LEN: usize = 12;
const RT_FIFO_LEN: usize = 188;
const RT_SPKCACHE_LEN: usize = 188;
const RT_RIGHT_CONTEXT: usize = 12;
...
/// enough has been buffered for one inference pass. With the current
/// low-latency settings, diarization latency is approximately
/// `(chunk_len + right_context) * 80ms`; increasing those values trades
/// responsiveness for potentially better turn-boundary accuracy.
...
impl DiarizationEngine {
    ...
    pub fn new(model_dir: &Path, use_cuda: bool, preset: &DiarizationPreset) -> Result<Self> {
        ...
        let diar_config = match preset {
            ...
            DiarizationPreset::Custom {
                onset,
                offset,
                min_seg_duration,
            } => {
                let mut cfg = parakeet_rs::sortformer::DiarizationConfig::custom(
                    *onset as f32,
                    *offset as f32,
                );
        ...
        let mut inner = {
            ...
            .with_context(|| {
                format!(
                    "loading Sortformer from {} (provider={provider})",
                    model_path.display()
                )
            })?
        };
        ...
        // Default Sortformer settings target much higher latency/offline-style
        // accuracy. Our runtime preset keeps FIFO/speaker-cache at the model's
        // full context while tuning chunk/right-context for the desired
        // real-time tradeoff.
    ...
    /// only when a full inference window is ready. The effective algorithmic
    /// latency is approximately `(chunk_len + right_context) * 80ms`.
    ///
    ...
    pub fn process_chunk(&mut self, pcm: &[f32]) -> Result<Option<DiarizationResult>> {
        let parakeet_segments = self.inner.feed(pcm).context("Sortformer feed error")?;
...
where
    F: FnOnce(
        &Path,
        parakeet_rs::sortformer::DiarizationConfig,
    ) -> Result<parakeet_rs::sortformer::Sortformer, anyhow::Error>,
{
    ...
    let cfg = diar_config;
    parakeet_rs::sortformer::Sortformer::with_config(model_path, Some(exec), cfg).with_context(
        || {
            format!(
                "loading Sortformer from {} (provider=Cpu)",
                model_path.display()
            )
        },
    )
